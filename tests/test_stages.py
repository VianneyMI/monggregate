...
class TestStages:
    ...
    def test_stage(self)->None:
        ...
        with pytest.raises(TypeError):
            ...
            stage = Stage(operand={})  # pylint: disable=abstract-class-instantiated
...
class TestStagesFunctional(TestStages):
    ...
    def test_bucket_auto_statement(self, state:State)->None:
        ...
        assert state["bucket_auto"].operand == Pipeline().bucket_auto(
            by = "test",
            buckets = 10
        )[-1].operand == {
            "$bucketAuto" : {
    ...
    def test_bucket_statement(self, state:State)->None:
        ...
        bucket = state["bucket"]
        assert bucket.operand == Pipeline().bucket(
            by = "income",
            boundaries = [25000, 40000, 60000, 100000])[-1].operand == {
            "$bucket" : {
                "groupBy" : "$income",
                "boundaries" : [25000, 40000, 60000, 100000],
                "default" : None,
    ...
    def test_count_statement(self, state:State)->None:
        ...
        count = state["count"]
        assert count.operand == Pipeline().count(name="count")[0].operand == {
            "$count" : "count"
        }
    ...
    def test_group_statement(self, state:State)->None:
        ...
        group = state["group"]
        assert group.operand == Pipeline().group(
            query = {
                "_id" :"count"
            }
        )[0].operand == {
    ...
    def test_limit_statement(self, state:State)->None:
        ...
        limit = state["limit"]
        assert limit.operand == Pipeline().limit(10)[0].operand == {
            "$limit" : 10
        }
    ...
    def test_lookup_statement(self, state:State)->None:
        ...
        lookup = state["lookup"]
        assert lookup.operand == Pipeline().lookup(
            right = "other_collection",
            left_on = "_id",
            right_on = "foreign_key",
            name = "matches"
    ...
    def test_match_statement(self, state:State)->None:
        ...
        match = state["match"]
        assert match.operand == Pipeline().match(
            query = {
                "_id":"12345"
            }
        )[0].operand == {
    ...
    def test_out_satement(self, state:State)->None:
        ...
        out = state["out"]
        assert out.operand == Pipeline().out("my_collection")[0].operand == {
            "$out" : "my_collection"
        }
    ...
    def test_project_statement(self, state:State)->None:
        ...
        assert project.operand == Pipeline().project(
            exclude = "_id"
        )[0].operand == {
            "$project" : {
                "_id" : 0
    ...
    def test_replace_root_statement(self, state:State)->None:
        ...
        assert replace_root.operand == Pipeline().replace_root(
            "myarray.mydocument"
        )[0].operand == {
            "$replaceRoot" : {
    ...
    def test_sample_statement(self, state:State) -> None:
        ...
        sample = state["sample"]
        assert sample.operand == Pipeline().sample(3)[0].operand == {
            "$sample" : {
                "size" : 3
            }
        }
    ...
    def test_set_statement(self, state:State) -> None:
        ...
        set = state["set"]
        assert set.operand == Pipeline().set(
            {
                "field1":"value1",
                "fieldN":"valueN"
            }
    ...
    def test_skip_statement(self, state:State)->None:
        ...
        skip = state["skip"]
        assert skip.operand == Pipeline().skip(10)[0].operand == {
            "$skip" : 10
        }
    ...
    def test_sort_by_count_statement(self, state:State)->None:
        ...
        assert sort_by_count.operand == Pipeline().sort_by_count(
            by = "name"
        )[0].operand == {
            "$sortByCount" : "$name"
        }
    ...
    def test_sort_statement(self, state:State)->None:
        ...
        sort = state["sort"]
        assert sort.operand == Pipeline().sort(field1=1, fieldN=-1)[0].operand == {
            "$sort" : {
                "field1" : 1,
                "fieldN" : -1
            }
    ...
    def test_unwind_statement(self, state:State)->None:
        ...
        unwind = state["unwind"]
        assert unwind.operand == Pipeline().unwind("xyz")[0].operand == {
            "$unwind" : {
                "path" : "$xyz"
            }
        }
