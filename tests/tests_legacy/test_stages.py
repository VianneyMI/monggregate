...
class TestStages:
    ...
    def test_bucket_auto(self, state: State) -> None:
        ...
        bucket_auto = BucketAuto(group_by="test", buckets=10)
        assert bucket_auto
    ...
    def test_bucket(self, state: State) -> None:
        ...
        assert Bucket(
            group_by="income", boundaries=[25000, 40000, 60000, 100000], default="other"
        )
    ...
    def test_group(self, state: State) -> None:
        ...
        # ------------------------
        group = Group(query={"_id": "count"})
        assert group
        state["group"] = group

        # Tests aliases
        # -----------------------
        assert Group(_id="count")

        # Test optional arguments
        # ------------------------
        assert Group(by="count", query={"output": {"$sum": "income"}})

        # Test by as list
        # ------------------------
        assert Group(by=["name", "age"], query={"output": {"$sum": "income"}})

        # Test by as set
        # ------------------------
        assert Group(by=set(["name", "age"]), query={"output": {"$sum": "income"}})

        # Test by as constant
        # ------------------------
        assert Group(by=1, query={"output": {"$sum": "income"}})

        # Test by as dict
        # ------------------------
        assert Group(by={"name": "$name"}, query={"output": {"$sum": "income"}})

        # Test by as None
        # ------------------------
        assert Group(by=None, query={"output": {"$sum": "income"}})
    ...
    def test_lookup(self, state: State) -> None:
        ...
        # -----------------------------
        uncorrelated = Lookup(right="other_collection", pipeline=[], name="new_fields")
        assert uncorrelated
    ...
    def test_project(self, state: State) -> None:
        ...
        # Testing aliases
        # -----------------------------
        assert Project(fields="email", include=True)

        # Testing optional attributes
        # -----------------------------
        # Include/Exclude as set
        assert Project(include=set(["name"]))

        assert Project(exclude=set(["password"]))
        # Include/Exclude as str or list of strings
        assert Project(include=set(["name"]))

        assert Project(exclude=["name", "_id"])
    ...
    def test_set(self, state: State) -> None:
        ...
        # Testing mandatory attributes
        # -----------------------------
        set = Set(document={"field1": "value1", "fieldN": "valueN"})
        assert set
        state["set"] = set
    ...
    def test_sort_by_count(self, state: State) -> None:
        ...
        # Testing mandatory attributes
        # -----------------------------
        sort_by_count = SortByCount(by="name")
    ...
    def test_sort(self, state: State) -> None:
        ...
        assert Sort(
            ascending=set(["field1", "field2"]), descending=set(["field3", "fieldN"])
        )

        # Further testing ascending and descending complex logic
        # ----------------------------
        assert Sort(by="year", ascending=True)

        assert Sort(ascending={"year": 1})
    ...
    def test_union_with(self, state: State) -> None:
        ...
        union_with = UnionWith(
            collection="other_collection", pipeline=Pipeline().match({"name": "test"})
        )
        assert union_with
    ...
    def test_unwind(self, state: State) -> None:
        ...
        # Testing aliases
        # -----------------------------
        assert Unwind(path="xyz")

        # Testing optional attributes
        # -----------------------------
        assert Unwind(path="xyz", include_array_index="index", always=True)
...
class TestStagesFunctional(TestStages):
    ...
    def test_bucket_auto_statement(self, state: State) -> None:
        ...
        assert (
            state["bucket_auto"].expression
            == Pipeline().bucket_auto(by="test", buckets=10)[-1].expression
            == {
                "$bucketAuto": {
                    "groupBy": "$test",
                    "buckets": 10,
    ...
    def test_bucket_statement(self, state: State) -> None:
        ...
        assert (
            bucket.expression
            == Pipeline()
            .bucket(by="income", boundaries=[25000, 40000, 60000, 100000])[-1]
            .expression
            == {
                "$bucket": {
                    "groupBy": "$income",
    ...
    def test_count_statement(self, state: State) -> None:
        ...
        assert (
            count.expression
            == Pipeline().count(name="count")[0].expression
            == {"$count": "count"}
        )
    ...
    def test_group_statement(self, state: State) -> None:
        ...
        assert (
            group.expression
            == Pipeline().group(query={"_id": "count"})[0].expression
            == {"$group": {"_id": "count"}}
        )
    ...
    def test_limit_statement(self, state: State) -> None:
        ...
        limit = state["limit"]
        assert limit.expression == Pipeline().limit(10)[0].expression == {"$limit": 10}
    ...
    def test_match_statement(self, state: State) -> None:
        ...
        assert (
            match.expression
            == Pipeline().match(query={"_id": "12345"})[0].expression
            == {"$match": {"_id": "12345"}}
        )
    ...
    def test_out_satement(self, state: State) -> None:
        ...
        assert (
            out.expression
            == Pipeline().out("my_collection")[0].expression
            == {"$out": "my_collection"}
        )
    ...
    def test_project_statement(self, state: State) -> None:
        ...
        assert (
            project.expression
            == Pipeline().project(exclude="_id")[0].expression
            == {"$project": {"_id": 0}}
        )
    ...
    def test_replace_root_statement(self, state: State) -> None:
        ...
        assert (
            replace_root.expression
            == Pipeline().replace_root("myarray.mydocument")[0].expression
            == {"$replaceRoot": {"newRoot": "$myarray.mydocument"}}
        )
    ...
    def test_sample_statement(self, state: State) -> None:
        ...
        assert (
            sample.expression
            == Pipeline().sample(3)[0].expression
            == {"$sample": {"size": 3}}
        )
    ...
    def test_set_statement(self, state: State) -> None:
        ...
        assert (
            set.expression
            == Pipeline().set({"field1": "value1", "fieldN": "valueN"})[0].expression
    ...
    def test_skip_statement(self, state: State) -> None:
        ...
        skip = state["skip"]
        assert skip.expression == Pipeline().skip(10)[0].expression == {"$skip": 10}
    ...
    def test_sort_by_count_statement(self, state: State) -> None:
        ...
        assert (
            sort_by_count.expression
            == Pipeline().sort_by_count(by="name")[0].expression
            == {"$sortByCount": "$name"}
        )
    ...
    def test_sort_statement(self, state: State) -> None:
        ...
        assert (
            sort.expression
            == Pipeline().sort(field1=1, fieldN=-1)[0].expression
            == {"$sort": {"field1": 1, "fieldN": -1}}
        )
    ...
    def test_unwind_statement(self, state: State) -> None:
        ...
        assert (
            unwind.expression
            == Pipeline().unwind("xyz")[0].expression
            == {"$unwind": {"path": "$xyz"}}
        )
